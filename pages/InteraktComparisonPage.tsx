...
import { interaktComparisonData } from '../data/pages/comparisons';
import { CheckIconSolid, XIconSolid } from '../icons';

interface InteraktComparisonPageProps {
    navigate: (path: string) => void;
}

const ComparisonTable: React.FC<{ data: typeof interaktComparisonData.comparisonTable }> = ({ data }) => {
    return (
            ...
            <table className="w-full min-w-[800px]">
                <thead>
                    <tr className="border-b-2 border-slate-200 dark:border-slate-700">
                        ...
                        <th className="p-4 text-left font-semibold text-slate-700 dark:text-slate-300 w-1/3">{interaktComparisonData.hero.competitorName}</th>
...
const InteraktComparisonPage: React.FC<InteraktComparisonPageProps> = ({ navigate }) => {
    const { hero, comparisonTable, tcoSection, onboardingSupport, summary } = interaktComparisonData;
    return (
        <main>
            <GenericPageHero 
                title={hero.title} 
