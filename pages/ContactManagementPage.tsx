...
import { contactManagementPageData } from '../data/pages/contactManagement';
...
const ContactManagementPage: React.FC<ContactManagementPageProps> = ({ navigate }) => {
    return (
        <main>
            <GenericPageHero title={contactManagementPageData.hero.title} subtitle={contactManagementPageData.hero.subtitle} />
            <div className="divide-y divide-slate-200/50 dark:divide-slate-800/50">
                {contactManagementPageData.sections.map((section, index) => (
            ...
            <div className="bg-pastel-green dark:bg-green-950/30 py-16 sm:py-20">
                <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
                    <ContentSection title={contactManagementPageData.closing.title} subtitle={contactManagementPageData.closing.subtitle}>
                        <a href="#/book-demo" onClick={(e) => { e.preventDefault(); navigate('#/book-demo'); }} className="mt-8 inline-block bg-green-500 text-white font-bold py-3 px-8 rounded-lg text-lg hover:bg-green-600 transition-transform hover:scale-105 shadow-lg">
                            {contactManagementPageData.closing.cta}
                        </a>
                    </ContentSection>
                </div>
            </div>
