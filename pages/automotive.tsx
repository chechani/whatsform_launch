...
import { automotivePageData } from '@/data/pages/automotive';

const AutomotivePage: React.FC = () => {

    return (
        <main>
            <GenericPageHero title={automotivePageData.hero.title} subtitle={automotivePageData.hero.subtitle} />
            
            <div className="divide-y divide-slate-200/50 dark:divide-slate-800/50">
                {automotivePageData.sections.map((section, index) => {
                    const subCategory = automotivePageData.subCategories.find(sc => sc.id === (section as any).id);
                    const bgColors = [
                                                ...
                                                <div>
                                                    ...
                                                    {subCategory && (
                                                        <div className="mt-8 text-center">
                                                            <Link href={`/automotive/${subCategory.slug}`} legacyBehavior>
            ...
            <section className="bg-green-50 dark:bg-green-950/30 py-16 sm:py-20">
                <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
                    <ContentSection title={automotivePageData.closing.title} subtitle={automotivePageData.closing.subtitle}>
                        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                            <Link href="/book-demo" className="inline-block bg-green-500 text-white font-bold py-3 px-8 rounded-lg text-lg hover:bg-green-600 transition-transform hover:scale-105 shadow-lg text-center">
                                {automotivePageData.closing.cta}
                            </Link>
