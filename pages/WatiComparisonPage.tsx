...
import { watiComparisonData } from '../data/pages/comparisons';
import { CheckIconSolid, XIconSolid } from '../icons';

interface WatiComparisonPageProps {
    navigate: (path: string) => void;
}

const ComparisonTable: React.FC<{ data: typeof watiComparisonData.comparisonTable }> = ({ data }) => {
    return (
            ...
            <table className="w-full min-w-[800px]">
                <thead>
                    <tr className="border-b-2 border-slate-200 dark:border-slate-700">
                        ...
                        <th className="p-4 text-left font-semibold text-slate-700 dark:text-slate-300 w-1/3">{watiComparisonData.hero.competitorName}</th>
...
const WatiComparisonPage: React.FC<WatiComparisonPageProps> = ({ navigate }) => {
    const { hero, comparisonTable, tcoSection, onboardingSupport, summary } = watiComparisonData;
    return (
        <main>
            <GenericPageHero 
                title={hero.title} 
